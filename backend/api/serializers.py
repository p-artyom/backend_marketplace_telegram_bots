...
class ShoppingCartSerializer(serializers.ModelSerializer):
    ...
    def get_total_cost(self, obj):
        cart = ShoppingCartItems.objects.filter(cart=obj, is_selected=True)
    ...
    def get_total_quantity(self, obj):
        return (
            ShoppingCartItems.objects.filter(cart=obj)
...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    def get_quantity(self, obj):
        ...
        item = OrderProductLists.objects.get(
            order__user=user,
            product=obj,
            order=instance,
        )
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        user = self.context.get('request').user
        cart = ShoppingCartItems.objects.filter(
            cart__owner=user,
            is_selected=True,
        )
        ...
        for item in cart:
            OrderProductLists.objects.create(
                order=order,
                product=item.item,
                quantity=item.quantity,
            )
        ShoppingCartItems.objects.filter(cart__owner=user).delete()
