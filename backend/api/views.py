...
class ProductAPIView(CRUDAPIView):
    ...
    def shopping_cart(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            cart_item, created = ShoppingCartItems.objects.get_or_create(
                cart=shopping_cart,
                item=product,
            )
            if not created:
        ...
        if request.method == 'DELETE':
            ...
            cart_item.delete()
            if not ShoppingCartItems.objects.filter(
                cart=shopping_cart,
            ).exists():
    ...
    def select(self, request, *args, **kwargs):
        ...
        serializer = ShoppingCartSerializer(shopping_cart, context=context)
        if request.method == 'PATCH':
            ShoppingCartItems.objects.filter(
                item=product,
                cart=shopping_cart,
    ...
    def select_all(self, request, *args, **kwargs):
        ...
        serializer = ShoppingCartSerializer(shopping_cart, context=context)
        if request.method == 'PATCH':
            ShoppingCartItems.objects.filter(cart=shopping_cart).update(
                is_selected=True,
            )
            return Response(serializer.data, status=status.HTTP_200_OK)
        if request.method == 'DELETE':
    ...
    def delete_all_selected(self, request, *args, **kwargs):
        ...
        if not ShoppingCartItems.objects.filter(cart=shopping_cart).exists():
